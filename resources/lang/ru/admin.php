<?php

return [
    'nav' => [
        'dashboard' => 'Панель управления',
        'users' => 'Пользователи',
        'products' => 'Товары',
        'orders' => 'Заказы',
        'media' => 'Медиатека',
        'categories' => 'Категории',
        'collections' => 'Коллекции',
        'options' => 'Опции',
        'brands' => 'Бренды',
        'settings' => 'Настройки',
        'documents' => 'Документы',
        'brand_requests' => 'Заявки',
        'my_categories' => 'Мои категории',
        'category_requests' => 'Запросы категорий',
        'moderation' => 'Модерация',
        'brand_page_requests' => 'Запросы брендов',
        'emails' => 'Письма',
        'my_page' => 'Моя страница',
    ],

    'user' => [
        'account' => 'Аккаунт',
        'logout' => 'Выйти',
    ],

    'theme' => [
        'light' => 'Светлая',
        'dark' => 'Тёмная',
        'system' => 'Системная',
        'toggle' => 'Переключить тему',
    ],

    'language' => [
        'switch' => 'Переключить язык',
        'russian' => 'Русский',
        'english' => 'English',
    ],

    'users' => [
        'title' => 'Пользователи',
        'description' => 'Управление пользователями и их правами',
    ],

    'users_table' => [
        'columns' => [
            'id' => 'ID',
            'name' => 'Имя',
            'role' => 'Роль',
            'created' => 'Создан',
            'created_at' => 'Создан',
        ],
        'roles' => [
            'user' => 'Пользователь',
            'admin' => 'Администратор',
            'manager' => 'Менеджер',
        ],
        'actions' => [
            'edit' => 'Редактировать',
            'delete' => 'Удалить',
            'open_menu' => 'Открыть меню',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'change_role' => 'Изменить роль...',
            'update_role' => 'Обновить роль',
            'delete_selected' => 'Удалить выбранные',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по имени или email...',
            'all_roles' => 'Все роли',
            'columns' => 'Колонки',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Пользователи не найдены.',
        'confirm' => [
            'delete_one' => 'Вы уверены, что хотите удалить',
            'delete_many' => 'Вы уверены, что хотите удалить',
            'users' => 'пользователей?',
        ],
        'messages' => [
            'deleted_success' => 'Пользователь удалён успешно',
            'deleted_failed' => 'Не удалось удалить пользователя',
            'deleted_many_success' => 'Успешно удалено',
            'deleted_many_users' => 'пользователей',
            'deleted_many_failed' => 'Не удалось удалить пользователей',
            'role_updated_success' => 'Роль обновлена для',
            'role_updated_users' => 'пользователей',
            'role_updated_failed' => 'Не удалось обновить роли',
        ],
    ],

    'users_edit' => [
        'title' => 'Редактирование пользователя',
        'description' => 'Изменение информации и прав пользователя',
        'fields' => [
            'first_name' => 'Имя',
            'last_name' => 'Фамилия',
            'email' => 'Email',
            'role' => 'Роль',
        ],
        'placeholders' => [
            'first_name' => 'Введите имя',
            'last_name' => 'Введите фамилию',
            'email' => 'user@example.com',
            'role' => 'Выберите роль',
        ],
        'roles' => [
            'none' => 'Пользователь (без роли)',
            'admin' => 'Администратор',
            'manager' => 'Менеджер',
        ],
        'required_fields' => 'Обязательные поля',
        'actions' => [
            'cancel' => 'Отмена',
            'save' => 'Сохранить изменения',
            'saving' => 'Сохранение...',
        ],
        'messages' => [
            'update_failed' => 'Не удалось обновить пользователя. Проверьте форму на наличие ошибок.',
        ],
        'brands' => [
            'title' => 'Бренды',
        ],
    ],

    'dashboard' => [
        'title' => 'Панель управления',
        'period' => 'Последние :days дней • :start — :end',
        'range_days' => ':days дней',
        'brand_account' => [
            'types' => [
                'master' => 'Мастер',
                'business' => 'Бизнес',
            ],
            'pending' => 'Заявка на смену типа на рассмотрении',
        ],
        'onboarding' => [
            'progress' => [
                'value' => ':completed/:total',
            ],
            'current' => [
                'categories' => 'Сейчас: заявка на категории',
                'settings' => 'Сейчас: настройки магазина',
                'product' => 'Сейчас: опубликуйте товар',
                'complete' => 'Основные шаги пройдены',
            ],
            'actions' => [
                'dismiss' => 'Скрыть обучение',
                'expand' => 'Показать прогресс',
                'continue' => 'Продолжить',
                'close_hint' => 'Закрыть подсказку',
                'go' => 'Перейти',
            ],
            'strip' => [
                'categories' => 'Категории',
                'settings' => 'Настройки',
                'product' => 'Первый товар',
                'waiting' => 'Ждём категорию',
            ],
            'hints' => [
                'step_1' => 'Шаг 1',
                'step_2' => 'Шаг 2',
                'step_3' => 'Шаг 3',
                'categories' => [
                    'title' => 'Откройте категории',
                    'description' => 'Отправьте заявку — после одобрения здесь появятся категории товаров.',
                    'description_rejected' => 'Заявку нужно отправить снова. Выберите подходящие категории и повторите запрос.',
                ],
                'settings' => [
                    'title' => 'Настройте магазин',
                    'description' => 'Заполните только нужное: описание, доставку, адреса или контакты.',
                ],
                'product' => [
                    'title' => 'Опубликуйте первый товар',
                    'description' => 'Создайте товар и включите его для показа на сайте.',
                    'description_blocked' => 'Этот шаг откроется после одобрения хотя бы одной категории.',
                ],
            ],
            'status' => [
                'completed' => 'Готово',
                'not_started' => 'Не начато',
            ],
            'categories' => [
                'status' => [
                    'not_submitted' => 'Обязательно',
                    'pending' => 'На проверке',
                    'approved' => 'Одобрено',
                    'partial' => 'Частично одобрено',
                    'rejected' => 'Нужно исправить',
                ],
            ],
            'errors' => [
                'update' => 'Не удалось сохранить прогресс. Попробуйте ещё раз.',
            ],
        ],
        'kpi' => [
            'revenue' => 'Выручка',
            'orders_amount' => 'Сумма заказов',
            'orders' => 'Заказы',
            'aov' => 'Средний чек',
            'units' => 'Продано единиц',
            'views' => 'Просмотры',
            'add_to_cart' => 'Добавления в корзину',
            'clicks' => 'Клики по карточкам',
            'conversion_view_to_click' => 'Просмотр → Клик',
            'paid_orders' => 'Оплаченные заказы',
            'unpaid_orders' => 'Неоплаченные заказы',
            'category_breakdown' => 'Категории',
            'category_breakdown_empty' => 'Просмотры по категориям появятся здесь',
            'category_other' => 'Остальные',
            'category_preview_1' => 'Мёд и варенье',
            'category_preview_2' => 'Соленья',
            'category_preview_3' => 'Печенье',
            'category_preview_4' => 'Чай',
            'conversion_view_to_atc' => 'Просмотр → В корзину',
            'conversion_view_to_order' => 'Просмотр → Заказ',
            'conversion_view_to_paid' => 'Просмотр → Оплата',
            'conversion_order_to_paid' => 'Заказ → Оплата',
        ],
        'charts' => [
            'toggle_sales' => 'Продажи',
            'toggle_interest' => 'Интерес',
            'sales_title' => 'Продажи по дням',
            'sales_description' => 'Выручка и количество заказов',
            'interest_title' => 'Интерес к товарам',
            'interest_description' => 'Просмотры, клики и добавления в корзину',
            'interest_description_no_sales' => 'Просмотры и клики по карточкам товаров',
        ],
        'tables' => [
            'top_title' => 'Топ товаров по продажам',
            'top_description' => 'Лидеры по выручке за период',
            'gap_title' => 'Топ конверсии',
            'gap_description' => 'Товары с наивысшей конверсией за период',
            'toggle_top' => 'Топ продаж',
            'toggle_gap' => 'Топ конверсии',
            'headers' => [
                'product' => 'Товар',
                'revenue' => 'Выручка',
                'orders' => 'Заказы',
                'ordered' => 'Заказали',
                'units' => 'Шт.',
                'views' => 'Просмотры',
                'add_to_cart' => 'В корзину',
                'added_to_cart' => 'Добавили в корзину',
                'conversion' => 'Просмотр → Заказ',
                'gap_views' => 'Просмотры',
                'gap_orders' => 'Заказы',
                'gap_conversion' => 'Конверсия',
                'view_to_cart' => 'Просмотр → В корзину',
                'view_to_order' => 'Просмотр → Заказ',
                'cart_to_order' => 'В корзину → Заказ',
            ],
            'pagination' => [
                'page_of' => 'Страница :page из :pages',
                'first' => 'Первая',
                'previous' => 'Предыдущая',
                'next' => 'Следующая',
                'last' => 'Последняя',
            ],
            'empty' => 'Нет данных за выбранный период',
        ],
        'labels' => [
            'no_category' => 'Без категории',
        ],
        'tooltip' => [
            'revenue' => 'Выручка',
            'orders' => 'Заказы',
            'views' => 'Просмотры',
            'clicks' => 'Клики',
            'add_to_cart' => 'В корзину',
        ],
        'orders_widget' => [
            'title' => 'Актуальность статусов',
            'description' => 'Показывает, где застряли заказы и что требует внимания',
            'health_label' => 'Свежесть статусов',
            'active_total' => ':count активных заказов',
            'needs_attention' => ':count требуют внимания',
            'unpaid_total' => 'Не оплачено',
            'unpaid_count' => 'Заказы: :count',
            'partial_paid_total' => 'Частично',
            'partial_paid_count' => 'Заказы: :count',
            'paid_total' => 'Оплачено',
            'paid_count' => 'Заказы: :count',
            'no_statuses' => 'Нет заказов за выбранный период',
            'cta' => 'Перейти к заказам',
            'attention' => [
                'pending' => 'Ожидает/подтверждён > :days дн.',
                'processing' => 'В обработке > :days дн.',
                'unpaid' => 'Не оплачен > :days дн.',
            ],
        ],
        'errors' => [
            'title' => 'Недостаточно данных',
            'no_brand' => 'Нет бренда, связанного с пользователем',
        ],
    ],

    'products' => [
        'title' => 'Товары',
        'description' => 'Управление каталогом товаров',
        'actions' => [
            'create' => 'Создать товар',
        ],
    ],

    'products_table' => [
        'columns' => [
            'title' => 'Название',
            'status' => 'Статус',
            'price' => 'Цена',
            'created' => 'Создан',
        ],
        'status' => [
            'enabled' => 'Активен',
            'disabled' => 'Неактивен',
        ],
        'actions' => [
            'edit' => 'Редактировать',
            'delete' => 'Удалить',
            'open_menu' => 'Открыть меню',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'change_status' => 'Изменить статус...',
            'update_status' => 'Обновить статус',
            'delete_selected' => 'Удалить выбранные',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по названию...',
            'all_statuses' => 'Все статусы',
            'columns' => 'Колонки',
            'price_from' => 'Цена от',
            'price_to' => 'Цена до',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Товары не найдены.',
        'confirm' => [
            'delete_one' => 'Вы уверены, что хотите удалить',
            'delete_many' => 'Вы уверены, что хотите удалить',
            'products' => 'товаров?',
        ],
        'success' => [
            'deleted' => 'Товар успешно удалён',
            'bulk_deleted' => 'Товары успешно удалены',
            'status_updated' => 'Статус товаров успешно обновлён',
        ],
        'errors' => [
            'delete_failed' => 'Не удалось удалить товар',
            'bulk_delete_failed' => 'Не удалось удалить товары',
            'status_update_failed' => 'Не удалось обновить статус товаров',
        ],
    ],

    'products_edit' => [
        'title' => 'Редактирование товара',
        'description' => 'Изменение информации о товаре и его вариантах',
        'view_on_store' => 'Посмотреть в магазине',
        'sections' => [
            'product_info' => 'Информация о товаре',
            'product_images' => 'Изображения товара',
            'variants' => 'Варианты товара',
        ],
        'no_images' => 'Нет загруженных изображений для этого товара',
        'images' => [
            'add_from_media' => 'Добавить из медиатеки',
            'upload_new' => 'Загрузить новые',
            'add_images' => 'Добавить изображения',
            'choose' => 'Выбрать',
            'no_images' => 'Нет изображений для этого товара',
            'position' => 'Позиция',
            'main' => 'Главное',
            'confirm_delete' => 'Отвязать это изображение от товара? Оно останется в медиатеке.',
        ],
        'variants' => [
            'confirm_remove_image' => 'Удалить изображение из этого варианта?',
        ],
        'variant_options' => [
            'title' => 'Значения опций',
            'select_placeholder' => 'Выберите значение',
        ],
        'fields' => [
            'title' => 'Название',
            'description' => 'Описание',
            'category' => 'Категория',
            'price' => 'Цена',
            'final_price' => 'Финальная цена',
            'enabled' => 'Товар активен',
            'variant_title' => 'Название варианта',
            'variant_enabled' => 'Вариант активен',
            'variant_image' => 'Изображение варианта',
            'made_to_order' => 'Под заказ',
            'production_time' => 'Срок изготовления, дней',
        ],
        'units' => [
            'days_short' => 'дн.',
        ],
        'placeholders' => [
            'title' => 'Введите название товара',
            'description' => 'Введите описание товара...',
            'category' => 'Выберите категорию',
            'price' => '0.00',
            'final_price' => '0.00',
            'variant_title' => 'например, Маленький, Средний, Красный, и т.д.',
        ],
        'category' => [
            'none' => 'Без категории',
            'not_in_list' => 'Категория не находится в списке категорий бренда. Невозможно редактировать этот товар.',
            'locked' => 'Категорию нельзя изменить после создания товара',
        ],
        'variant' => 'Вариант',
        'required_fields' => 'Обязательные поля',
        'actions' => [
            'back' => 'Назад к товарам',
            'add_variant' => 'Добавить вариант',
            'cancel' => 'Отмена',
            'discard' => 'Отменить',
            'save' => 'Сохранить изменения',
            'saving' => 'Сохранение...',
            'select_image' => 'Выбрать изображение',
            'change_image' => 'Изменить изображение',
        ],
        'unsaved' => [
            'message' => 'Есть несохранённые изменения',
            'mobile_message' => 'Не сохранено',
            'dialog' => [
                'title' => 'Несохранённые изменения',
                'description' => 'У вас есть несохранённые изменения. Что вы хотите сделать?',
                'save' => 'Сохранить и уйти',
                'discard' => 'Отменить изменения',
                'cancel' => 'Остаться на странице',
            ],
        ],
        'errors' => [
            'at_least_one_variant' => 'У товара должен быть хотя бы один вариант',
            'save_variant_first' => 'Пожалуйста, сначала сохраните товар, чтобы добавить изображения для вариантов',
            'image_not_attached' => 'Это изображение не прикреплено к товару. Сначала прикрепите его.',
            'all_combinations_used' => 'Все возможные комбинации опций уже использованы. Невозможно добавить больше вариантов.',
            'duplicate_variants' => 'Невозможно сохранить товар с дублирующимися вариантами. Убедитесь, что каждый вариант имеет уникальную комбинацию опций.',
            'save_failed' => 'Не удалось сохранить товар',
            'image_attach_failed' => 'Не удалось прикрепить изображения',
            'image_upload_failed' => 'Не удалось загрузить изображения',
            'image_reorder_failed' => 'Не удалось изменить порядок изображений',
            'image_detach_failed' => 'Не удалось удалить изображение',
            'variant_image_failed' => 'Не удалось обновить изображение варианта',
        ],
        'hints' => [
            'save_to_add_image' => 'Сохраните товар, чтобы добавить изображения',
        ],
        'success' => [
            'saved' => 'Товар успешно сохранён',
        ],
        'duplicate_variants' => [
            'title' => 'Обнаружены дублирующиеся варианты',
            'description' => 'Следующие варианты имеют одинаковые значения опций. Каждый вариант должен иметь уникальную комбинацию опций.',
            'description_short' => 'Дублирующиеся комбинации:',
            'group_label' => 'Варианты',
            'line_message' => 'Этот вариант является дубликатом. Измените значения опций или удалите его.',
        ],
        'basic_info' => 'Основная информация',
        'product_title_label' => 'Название продукта',
        'category_options_header' => 'Категория и опции',
        'select_all' => 'Выбрать все',
        'deselect_all' => 'Снять все',
        'generate_variants' => 'Сгенерировать варианты',
        'combinations_count' => 'комб.',
        'new_variant_header' => 'Новый вариант',
        'preview_label' => 'Превью:',
        'duplicate_label' => ' — уже существует',
        'bulk_pricing_header' => 'Массовое ценообразование',
        'sale_price_label' => 'Цена продажи',
        'compare_price_label' => 'Цена до скидки',
        'price_ph' => 'Цена',
        'old_price_ph' => 'Старая',
        'apply_to_all' => 'Применить ко всем',
        'add_btn' => 'Добавить',
        'delete_btn' => 'Удалить',
        'select_value_for' => 'Выберите значение для:',
        'no_variants_message' => 'Нет вариантов. Нажмите «Добавить вариант»',
        'no_options_message' => 'У выбранной категории нет опций',
        'select_category_variants' => 'Выберите категорию, чтобы добавить варианты',
        'images_header' => 'Изображения',
        'no_images_short' => 'Нет изображений',
        'drop_images_here' => 'Перетащите изображения сюда',
        'drop_or_click' => 'или нажмите для выбора файлов · JPG, PNG, WebP',
        'make_main_title' => 'Сделать главным',
        'library_badge' => 'библ.',
        'confirm_delete_title' => 'Удалить изображение',
        'confirm_delete_local_image' => 'Изображение будет убрано из товара. Оно останется в медиатеке.',
        'confirm_delete_cancel' => 'Отмена',
        'main_badge' => 'ГЛАВНОЕ',
        'show_more' => 'Показать ещё',
        'show_less' => 'Скрыть',
        'summary_header' => 'СВОДКА',
        'summary_name' => 'Название',
        'summary_category' => 'Категория',
        'summary_images' => 'Изображения',
        'summary_variants' => 'Вариантов',
        'summary_price' => 'Цена',
        'summary_discounted' => 'Со скидкой',
        'summary_var_short' => 'вар.',
        'delete_variant_confirm' => 'Удалить вариант #',
        'toast_updated' => 'Продукт обновлён',
        'toast_error_save' => 'Ошибка при сохранении',
        'validation' => [
            'title_required' => 'Введите название',
            'category_required' => 'Выберите категорию',
            'at_least_one_active_variant' => 'Нужен хотя бы один активный вариант',
            'all_variants_need_price' => 'У всех активных вариантов должна быть цена',
            'compare_price_must_be_higher' => 'Цена до скидки должна быть выше текущей',
            'duplicate_combination' => 'Такая комбинация уже существует',
        ],
    ],

    'products_create' => [
        'title' => 'Создание товара',
        'description' => 'Добавление нового товара в каталог',
        'sections' => [
            'product_info' => 'Информация о товаре',
        ],
        'fields' => [
            'title' => 'Название',
            'description' => 'Описание',
            'category' => 'Категория',
            'price' => 'Цена',
            'final_price' => 'Финальная цена',
            'enabled' => 'Товар активен',
            'variant_title' => 'Название варианта',
            'variant_enabled' => 'Вариант активен',
        ],
        'placeholders' => [
            'title' => 'Введите название товара',
            'description' => 'Введите описание товара...',
            'category' => 'Выберите категорию',
            'price' => '0.00',
            'final_price' => '0.00',
            'variant_title' => 'например, Маленький, Средний, Красный, и т.д.',
        ],
        'category' => [
            'none' => 'Без категории',
        ],
        'category_picker' => [
            'label' => 'Категория',
            'hint' => 'Выберите категорию. Категория задаётся только при создании товара — после изменить её нельзя.',
            'search_placeholder' => 'Поиск категорий...',
        ],
        'no_categories' => [
            'title' => 'У вашего бренда пока нет категорий',
            'description' => 'Создать товар пока нельзя: он создаётся в одной из категорий бренда. Отправьте запрос на нужные категории — после одобрения администратором можно будет добавлять товары.',
            'action' => 'Запросить категории',
            'validation' => 'Товар можно создать только в категории вашего бренда. Отправьте запрос на категории в разделе «Мои категории».',
        ],
        'change_category_confirm' => [
            'title' => 'Сменить категорию?',
            'description' => 'Существующие варианты будут удалены и заменены на дефолтный вариант для новой категории.',
            'confirm' => 'Сменить категорию',
        ],
        'required_fields' => 'Обязательные поля',
        'actions' => [
            'back' => 'Назад к товарам',
            'add_variant' => 'Добавить вариант',
            'cancel' => 'Отмена',
            'create' => 'Создать товар',
            'creating' => 'Создание...',
        ],
        'errors' => [
            'at_least_one_variant' => 'У товара должен быть хотя бы один вариант',
            'all_combinations_used' => 'Все возможные комбинации опций уже использованы. Невозможно добавить больше вариантов.',
        ],
        'success' => [
            'created' => 'Товар успешно создан',
        ],
        'duplicate_variants' => [
            'title' => 'Обнаружены дублирующиеся варианты',
            'description' => 'Следующие варианты имеют одинаковые значения опций. Каждый вариант должен иметь уникальную комбинацию опций.',
            'description_short' => 'Дублирующиеся комбинации:',
            'group_label' => 'Варианты',
            'line_message' => 'Этот вариант является дубликатом. Измените значения опций или удалите его.',
        ],
        'toast_error_create' => 'Ошибка при создании',
    ],

    'orders' => [
        'title' => 'Заказы',
        'description' => 'Управление заказами клиентов',
    ],

    'orders_table' => [
        'columns' => [
            'order' => 'Заказ',
            'customer' => 'Клиент',
            'total' => 'Сумма',
            'status' => 'Статус',
            'delivery' => 'Доставка',
            'payment' => 'Оплата',
            'created' => 'Создан',
        ],
        'items' => 'товар(ов)',
        'status' => [
            'pending' => 'Ожидает',
            'confirmed' => 'Подтверждён',
            'processing' => 'В обработке',
            'completed' => 'Выполнен',
            'cancelled' => 'Отменён',
            'archived' => 'Архив',
        ],
        'delivery_status' => [
            'pending' => 'Ожидает',
            'packing' => 'Упаковка',
            'shipped' => 'Отправлен',
            'transit' => 'В пути',
            'delivered' => 'Доставлен',
        ],
        'financial_status' => [
            'unpaid' => 'Не оплачен',
            'paid' => 'Оплачен',
            'partial_paid' => 'Частично',
            'refunded' => 'Возврат',
        ],
        'actions' => [
            'view' => 'Просмотр',
            'open_menu' => 'Открыть меню',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по номеру заказа...',
            'all_statuses' => 'Все статусы',
            'all_payments' => 'Все оплаты',
            'columns' => 'Колонки',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Заказы не найдены.',
    ],

    'order_show' => [
        'title' => 'Детали заказа',
        'back' => 'Назад к заказам',
        'sections' => [
            'customer' => 'Клиент',
            'items' => 'Заказ',
            'shipping' => 'Доставка',
            'timeline' => 'История заказа',
        ],
        'customer' => [
            'name' => 'Имя',
            'email' => 'Email',
            'phone' => 'Телефон',
            'no_customer' => 'Нет информации о клиенте',
        ],
        'items' => [
            'product' => 'Товар',
            'variant' => 'Вариант',
            'quantity' => 'Кол-во',
            'price' => 'Цена',
            'total' => 'Итого',
            'no_items' => 'Нет товаров в заказе',
        ],
        'shipping' => [
            'name' => 'Получатель',
            'phone' => 'Телефон',
            'note' => 'Примечание',
            'method' => 'Способ доставки',
            'no_shipping' => 'Нет информации о доставке',
            'price' => 'Стоимость доставки',
            'price_placeholder' => 'Сумма',
            'price_save' => 'Сохранить',
            'price_updated' => 'Стоимость доставки обновлена',
            'edit' => 'Изменить',
            'save' => 'Сохранить',
            'cancel' => 'Отменить',
            'empty' => '—',
            'updated' => 'Информация о доставке обновлена',
            'update_failed' => 'Не удалось обновить информацию о доставке',
            'method_change_price_hint' => 'Стоимость доставки и итог заказа будут пересчитаны по выбранному способу',
        ],
        'timeline' => [
            'event' => [
                'created' => 'Создан',
                'updated' => 'Обновлён',
                'deleted' => 'Удалён',
            ],
            'by' => '',
            'system' => 'Система',
            'no_history' => 'История недоступна',
            'show_all' => 'Показать ещё :count',
            'show_less' => 'Свернуть историю',
        ],
        'totals' => [
            'subtotal' => 'Подытог',
            'items' => 'Товары',
            'discount' => 'Скидка',
            'shipping' => 'Доставка',
            'shipping_negotiable' => 'Согласуется с покупателем',
            'total' => 'Итого',
        ],
        'note' => 'Примечание',
        'no_note' => 'Нет примечания',
        'created_at' => 'Создан',
        'updated_at' => 'Обновлён',
        'status_updated' => 'Статус успешно обновлён',
        'status_update_failed' => 'Не удалось обновить статус',
    ],

    'image_upload' => [
        'title' => 'Загрузка изображений',
        'description' => 'Загрузите новые изображения для вашего товара',
        'select_or_drop' => 'Нажмите для выбора или перетащите файлы сюда',
        'max_files' => 'Максимум :max файлов',
        'files_selected' => 'Выбрано файлов: :count',
        'selected_files' => 'Выбранные файлы',
    ],

    'media_picker' => [
        'title' => 'Выбор изображений',
        'description_multiple' => 'Выберите одно или несколько изображений из медиатеки',
        'description_single' => 'Выберите изображение из медиатеки',
        'search_placeholder' => 'Поиск изображений...',
        'no_results' => 'Изображения не найдены',
        'no_images' => 'Нет изображений в медиатеке',
        'select_button' => 'Выбрать (:count)',
    ],

    'image_picker' => [
        'title' => 'Выбор изображений',
        'description' => 'Выберите существующие изображения или загрузите новые',
        'tab_library' => 'Медиатека',
        'tab_upload' => 'Загрузка',
        'search_placeholder' => 'Поиск изображений...',
        'no_results' => 'Изображения не найдены',
        'no_images' => 'Нет изображений в медиатеке',
        'select_button' => 'Выбрать (:count)',
        'select_or_drop' => 'Нажмите для выбора или перетащите файлы сюда',
        'max_files' => 'Максимум :max файлов',
        'files_selected' => 'Выбрано файлов: :count',
        'selected_files' => 'Выбранные файлы',
        'load_more' => 'Загрузить ещё',
        'load_failed' => 'Не удалось загрузить изображения',
    ],

    'product_images_picker' => [
        'title' => 'Выбор изображения товара',
        'description' => 'Выберите изображение из изображений товара',
        'no_images' => 'Нет изображений для этого товара',
        'add_images_first' => 'Сначала добавьте изображения к товару',
        'position' => 'Позиция',
        'main' => 'Главное',
        'select_button' => 'Выбрать изображение',
    ],

    'media' => [
        'title' => 'Медиатека',
        'upload_button' => 'Загрузить изображения',
        'upload_dialog_title' => 'Загрузить изображения',
        'upload_dialog_description' => 'Выберите до 50 изображений для загрузки в медиатеку.',
        'select_files' => 'Нажмите для выбора файлов',
        'max_files' => 'Максимум 50 файлов, до 10 МБ каждый',
        'max_files_error' => 'Вы можете загрузить максимум 50 файлов за раз',
        'no_files_selected' => 'Пожалуйста, выберите файлы для загрузки',
        'files_selected' => 'Выбрано файлов: :count',
        'selected_files' => 'Выбранные файлы',
        'delete_confirm_title' => 'Удалить изображение',
        'delete_confirm_description' => 'Вы уверены, что хотите удалить это изображение? Это действие нельзя отменить.',
        'empty_title' => 'Нет изображений',
        'empty_description' => 'Загрузите ваши первые изображения, чтобы начать работу с медиатекой.',
        'upload_first_images' => 'Загрузить изображения',
    ],

    'image_uploader' => [
        'drop_or_click_multiple' => 'Перетащите файлы сюда или нажмите для выбора',
        'drop_or_click_single' => 'Перетащите файл сюда или нажмите для выбора',
        'hint_multiple' => ':formats — макс. :size — до :max файлов',
        'hint_single' => ':formats — макс. :size',
        'upload_failed' => 'Не удалось загрузить изображения',
        'invalid_files_skipped' => 'Некоторые файлы были пропущены (неверный формат или размер)',
    ],

    'common' => [
        'cancel' => 'Отмена',
        'delete' => 'Удалить',
        'deleting' => 'Удаление...',
        'upload' => 'Загрузить',
        'uploading' => 'Загрузка...',
        'save' => 'Сохранить',
        'saving' => 'Сохранение...',
    ],

    'pagination' => [
        'previous' => 'Предыдущая',
        'next' => 'Следующая',
        'page_of' => 'Страница :current из :total',
        'showing' => 'Показано с :from по :to из :total',
    ],

    'brand_requests' => [
        'title' => 'Заявки на бренд',
        'description' => 'Управление заявками на создание бренда',
    ],

    'brand_requests_table' => [
        'columns' => [
            'id' => 'ID',
            'email' => 'Email',
            'phone' => 'Телефон',
            'name' => 'Имя',
            'brand_name' => 'Название бренда',
            'status' => 'Статус',
            'created' => 'Создана',
        ],
        'status' => [
            'pending' => 'Ожидает',
            'approved' => 'Одобрена',
            'rejected' => 'Отклонена',
        ],
        'actions' => [
            'view' => 'Просмотр',
            'open_menu' => 'Открыть меню',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по email или имени...',
            'all_statuses' => 'Все статусы',
            'columns' => 'Колонки',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Заявки не найдены.',
    ],

    'brand_request_show' => [
        'title' => 'Заявка на бренд',
        'sections' => [
            'request_info' => 'Информация о заявке',
            'status_info' => 'Информация о статусе',
            'linked_entities' => 'Связанные объекты',
        ],
        'fields' => [
            'email' => 'Email',
            'phone' => 'Телефон',
            'name' => 'Контактное имя',
            'brand_name' => 'Запрашиваемое название бренда',
            'employment_type' => 'Форма занятости',
            'status' => 'Статус',
            'created_at' => 'Дата создания',
            'updated_at' => 'Дата обновления',
            'approved_at' => 'Дата одобрения',
            'rejected_at' => 'Дата отклонения',
            'reviewed_by' => 'Рассмотрел',
            'rejection_reason' => 'Причина отклонения',
            'created_user' => 'Созданный пользователь',
            'created_brand' => 'Созданный бренд',
        ],
        'actions' => [
            'resend_password_setup' => 'Выслать ссылку для пароля',
            'approve' => 'Одобрить',
            'reject' => 'Отклонить',
        ],
        'reject_modal' => [
            'title' => 'Отклонение заявки',
            'description' => 'Укажите причину отклонения заявки. Эта информация будет видна заявителю.',
            'reason_label' => 'Причина отклонения',
            'reason_placeholder' => 'Введите причину отклонения...',
            'cancel' => 'Отмена',
            'confirm' => 'Отклонить заявку',
        ],
    ],

    'emails' => [
        'title' => 'Письма',
        'description' => 'Очередь исходящих писем и история отправки',
    ],

    'emails_table' => [
        'columns' => [
            'id' => 'ID',
            'recipient' => 'Получатель',
            'subject' => 'Тема',
            'template' => 'Шаблон',
            'status' => 'Статус',
            'attempts' => 'Попытки',
            'scheduled_at' => 'Запланировано',
            'sent_at' => 'Отправлено',
        ],
        'status' => [
            'pending' => 'В очереди',
            'processing' => 'Отправляется',
            'sent' => 'Отправлено',
            'failed' => 'Ошибка',
        ],
        'templates' => [
            'order_created_user' => 'Заказ создан (клиент)',
            'order_created_admin' => 'Новый заказ (админ)',
            'brand_request_created_user' => 'Заявка на бренд создана (клиент)',
            'brand_request_created_admin' => 'Новая заявка на бренд (админ)',
            'brand_request_approved_user' => 'Заявка на бренд одобрена (клиент)',
            'brand_request_rejected_user' => 'Заявка на бренд отклонена (клиент)',
            'category_request_created_user' => 'Заявка на категории создана (клиент)',
            'category_request_created_admin' => 'Новая заявка на категории (админ)',
            'category_request_reviewed_user' => 'Заявка на категории рассмотрена (клиент)',
        ],
        'actions' => [
            'open_menu' => 'Открыть меню',
            'view' => 'Подробнее',
            'retry' => 'Повторить',
            'resend' => 'Переотправить',
            'cancel' => 'Отменить',
            'delete' => 'Удалить',
        ],
        'confirm' => [
            'cancel' => 'Отменить отправку этого письма?',
            'resend' => 'Отправить копию этого письма на :email?',
            'delete' => 'Удалить это письмо? Действие необратимо.',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по email или теме...',
            'all_statuses' => 'Все статусы',
            'all_templates' => 'Все шаблоны',
            'columns' => 'Колонки',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Писем не найдено.',
    ],

    'email_show' => [
        'title' => 'Письмо',
        'sections' => [
            'meta' => 'Информация о письме',
            'delivery' => 'Доставка',
            'error' => 'Ошибка отправки',
            'preview' => 'Превью',
            'payload' => 'Payload',
        ],
        'fields' => [
            'subject' => 'Тема',
            'template' => 'Шаблон',
            'recipient' => 'Получатель',
            'sender' => 'Отправитель',
            'sender_default' => 'По умолчанию (из конфига)',
            'reply_to' => 'Reply-To',
            'locale' => 'Локаль',
            'status' => 'Статус',
            'attempts' => 'Попытки',
            'scheduled_at' => 'Запланировано',
            'sent_at' => 'Отправлено',
            'created_at' => 'Создано',
            'updated_at' => 'Обновлено',
            'source' => 'Источник',
            'resend' => 'Resend',
            'resend_open' => 'Открыть в Resend',
        ],
        'actions' => [
            'send_copy' => 'Отправить копию мне',
            'resend' => 'Переотправить',
            'delete' => 'Удалить',
        ],
        'confirm' => [
            'resend' => 'Отправить копию этого письма на :email?',
            'delete' => 'Удалить это письмо? Действие необратимо.',
        ],
        'preview_unavailable' => 'Шаблон не найден или не может быть отрисован.',
    ],

    'brands' => [
        'title' => 'Бренды',
        'description' => 'Управление брендами',
        'actions' => [
            'create' => 'Создать бренд',
        ],
    ],

    'brands_table' => [
        'types' => [
            'master' => 'Мастер',
            'business' => 'Бизнес',
        ],
        'columns' => [
            'type' => 'Тип',
            'logo' => 'Логотип',
            'id' => 'ID',
            'name' => 'Название',
            'owner' => 'Владелец',
            'products' => 'Товары',
            'created' => 'Создан',
        ],
        'actions' => [
            'edit' => 'Редактировать',
            'delete' => 'Удалить',
            'open_menu' => 'Открыть меню',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'delete_selected' => 'Удалить выбранные',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по названию...',
            'columns' => 'Колонки',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Бренды не найдены.',
        'confirm' => [
            'delete_one' => 'Вы уверены, что хотите удалить',
            'delete_many' => 'Вы уверены, что хотите удалить',
            'brands' => 'брендов?',
        ],
        'success' => [
            'deleted' => 'Бренд успешно удалён',
            'bulk_deleted' => 'Бренды успешно удалены',
        ],
        'errors' => [
            'delete_failed' => 'Не удалось удалить бренд',
            'bulk_delete_failed' => 'Не удалось удалить бренды',
        ],
    ],

    'brands_edit' => [
        'account_type' => [
            'title' => 'Тип аккаунта',
            'description' => 'Мастер — частный продавец, бизнес — компания. Меняет только администратор.',
        ],
        'page' => [
            'title' => 'Страница бренда',
            'description' => 'Показ персональной страницы на витрине. Управляет только администратор.',
            'switch' => 'Страница опубликована',
            'hint_published' => 'Страница открыта покупателям.',
            'hint_blocked' => 'Показ выключен: страница отдаёт 404.',
            'published_at' => 'Опубликована',
            'open' => 'Открыть страницу',
            'published' => 'Страница бренда опубликована.',
            'blocked' => 'Показ страницы бренда выключен.',
            'domain_required' => 'Сначала задайте домен: без него у страницы нет адреса.',
        ],
        'domain' => [
            'title' => 'Домен страницы',
            'description' => 'Хэндл в адресе страницы бренда на витрине: /brand/<домен>. Настраивает только администратор.',
            'label' => 'Домен',
            'save' => 'Сохранить домен',
            'saved' => 'Домен сохранён.',
            'hint_current' => 'Текущий домен бренда — часть адреса /brand/<домен>.',
            'hint_form' => 'Сохраняется вместе с остальными полями бренда.',
            'hint_suggested' => 'Предложен по названию бренда. Станет частью адреса: /brand/<домен>.',
            'invalid' => 'Домен может содержать только латиницу, цифры и дефис.',
            'taken' => 'Этот домен уже занят другим брендом.',
        ],
        'title' => 'Редактирование бренда',
        'description' => 'Изменение информации о бренде',
        'fields' => [
            'name' => 'Название',
            'description' => 'Описание',
            'employment_type' => 'Форма занятости',
            'employment_type_none' => 'Не указана',
            'employment_type_unknown' => 'Неизвестное значение',
        ],
        'employment_type_description' => 'Как продавец оформлен юридически. Поле необязательное и меняется только администратором.',
        'placeholders' => [
            'name' => 'Введите название бренда',
            'description' => 'Введите описание бренда...',
        ],
        'required_fields' => 'Обязательные поля',
        'actions' => [
            'cancel' => 'Отмена',
            'save' => 'Сохранить изменения',
            'saving' => 'Сохранение...',
        ],
        'categories_save' => 'Сохранить',
        'categories_saving' => 'Сохранение...',
        'shipping_methods' => 'Способы доставки',
        'shipping_methods_description' => 'Способы доставки, подключённые магазином в «Настройки → Доставка»',
        'no_shipping_methods' => 'Магазин ещё не подключил способы доставки',
    ],

    'brands_create' => [
        'title' => 'Создание бренда',
        'description' => 'Добавление нового бренда',
        'actions' => [
            'create' => 'Создать бренд',
            'creating' => 'Создание...',
        ],
    ],

    'collections' => [
        'title' => 'Коллекции',
        'description' => 'Управление коллекциями товаров',
        'actions' => [
            'create' => 'Создать коллекцию',
        ],
    ],

    'collections_table' => [
        'columns' => [
            'id' => 'ID',
            'name' => 'Название',
            'title' => 'Заголовок',
            'categories' => 'Категории',
            'created' => 'Создана',
        ],
        'actions' => [
            'edit' => 'Редактировать',
            'delete' => 'Удалить',
            'open_menu' => 'Открыть меню',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'delete_selected' => 'Удалить выбранные',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по названию...',
            'columns' => 'Колонки',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Коллекции не найдены.',
        'confirm' => [
            'delete_one' => 'Вы уверены, что хотите удалить',
            'delete_many' => 'Вы уверены, что хотите удалить',
            'collections' => 'коллекций?',
        ],
        'success' => [
            'deleted' => 'Коллекция успешно удалена',
            'bulk_deleted' => 'Коллекции успешно удалены',
        ],
        'errors' => [
            'delete_failed' => 'Не удалось удалить коллекцию',
            'bulk_delete_failed' => 'Не удалось удалить коллекции',
        ],
    ],

    'collections_edit' => [
        'title' => 'Редактирование коллекции',
        'description' => 'Изменение информации о коллекции',
        'fields' => [
            'name' => 'Название',
            'handle' => 'Идентификатор',
            'title' => 'Заголовок',
            'header' => 'Шапка',
            'description' => 'Описание',
        ],
        'placeholders' => [
            'name' => 'Введите название коллекции',
            'handle' => 'например, letnyaya-rasprodazha',
            'title' => 'Введите заголовок коллекции',
            'header' => 'Введите шапку коллекции',
            'description' => 'Введите описание коллекции...',
        ],
        'hints' => [
            'handle' => 'Оставьте пустым для автоматической генерации из названия',
        ],
        'required_fields' => 'Обязательные поля',
        'actions' => [
            'cancel' => 'Отмена',
            'save' => 'Сохранить изменения',
            'saving' => 'Сохранение...',
        ],
    ],

    'collections_create' => [
        'title' => 'Создание коллекции',
        'description' => 'Добавление новой коллекции',
        'actions' => [
            'create' => 'Создать коллекцию',
            'creating' => 'Создание...',
        ],
    ],

    'categories' => [
        'title' => 'Категории',
        'description' => 'Управление категориями товаров',
        'actions' => [
            'create' => 'Создать категорию',
        ],
    ],

    'categories_table' => [
        'columns' => [
            'id' => 'ID',
            'name' => 'Название',
            'title' => 'Заголовок',
            'products' => 'Товары',
            'created' => 'Создана',
        ],
        'actions' => [
            'edit' => 'Редактировать',
            'delete' => 'Удалить',
            'open_menu' => 'Открыть меню',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'delete_selected' => 'Удалить выбранные',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по названию...',
            'columns' => 'Колонки',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Категории не найдены.',
        'confirm' => [
            'delete_one' => 'Вы уверены, что хотите удалить',
            'delete_many' => 'Вы уверены, что хотите удалить',
            'categories' => 'категорий?',
        ],
        'success' => [
            'deleted' => 'Категория успешно удалена',
            'bulk_deleted' => 'Категории успешно удалены',
        ],
        'errors' => [
            'delete_failed' => 'Не удалось удалить категорию',
            'bulk_delete_failed' => 'Не удалось удалить категории',
        ],
    ],

    'categories_edit' => [
        'title' => 'Редактирование категории',
        'description' => 'Изменение информации о категории',
        'fields' => [
            'name' => 'Название',
            'handle' => 'Идентификатор',
            'title' => 'Заголовок',
            'header' => 'Шапка',
            'description' => 'Описание',
        ],
        'placeholders' => [
            'name' => 'Введите название категории',
            'handle' => 'например, letnie-platya',
            'title' => 'Введите заголовок категории',
            'header' => 'Введите шапку категории',
            'description' => 'Введите описание категории...',
        ],
        'hints' => [
            'handle' => 'Оставьте пустым для автоматической генерации из названия',
        ],
        'required_fields' => 'Обязательные поля',
        'actions' => [
            'cancel' => 'Отмена',
            'save' => 'Сохранить изменения',
            'saving' => 'Сохранение...',
        ],
    ],

    'categories_create' => [
        'title' => 'Создание категории',
        'description' => 'Добавление новой категории',
        'actions' => [
            'create' => 'Создать категорию',
            'creating' => 'Создание...',
        ],
    ],

    'options' => [
        'title' => 'Опции',
        'description' => 'Управление опциями товаров (Размер, Цвет и т.д.)',
        'actions' => [
            'create' => 'Создать опцию',
        ],
    ],

    'options_table' => [
        'columns' => [
            'id' => 'ID',
            'name' => 'Название',
            'values' => 'Значения',
            'created' => 'Создана',
        ],
        'actions' => [
            'edit' => 'Редактировать',
            'delete' => 'Удалить',
            'open_menu' => 'Открыть меню',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'delete_selected' => 'Удалить выбранные',
            'clear_selection' => 'Очистить выбор',
        ],
        'filters' => [
            'search_placeholder' => 'Поиск по названию...',
            'columns' => 'Колонки',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Опции не найдены.',
        'confirm' => [
            'delete_one' => 'Вы уверены, что хотите удалить',
            'delete_many' => 'Вы уверены, что хотите удалить',
            'options' => 'опций?',
        ],
        'success' => [
            'deleted' => 'Опция успешно удалена',
            'bulk_deleted' => 'Опции успешно удалены',
        ],
        'errors' => [
            'delete_failed' => 'Не удалось удалить опцию',
            'bulk_delete_failed' => 'Не удалось удалить опции',
        ],
    ],

    'options_edit' => [
        'title' => 'Редактирование опции',
        'description' => 'Изменение информации об опции',
        'fields' => [
            'name' => 'Название',
            'title' => 'Заголовок',
            'description' => 'Описание',
        ],
        'placeholders' => [
            'name' => 'например, Размер, Цвет',
            'title' => 'Введите заголовок опции',
            'description' => 'Введите описание опции...',
        ],
        'values' => [
            'title' => 'Значения опции',
            'description' => 'Добавьте значения для этой опции (например, S, M, L для Размера)',
            'add' => 'Добавить значение',
            'add_first' => 'Добавить первое значение',
            'empty' => 'Значения ещё не добавлены',
            'placeholder' => 'Введите значение (например, Маленький, Красный, 42)',
        ],
        'required_fields' => 'Обязательные поля',
        'actions' => [
            'cancel' => 'Отмена',
            'save' => 'Сохранить изменения',
            'saving' => 'Сохранение...',
        ],
    ],

    'options_create' => [
        'title' => 'Создание опции',
        'description' => 'Добавление новой опции',
        'actions' => [
            'create' => 'Создать опцию',
            'creating' => 'Создание...',
        ],
    ],

    'settings_tabs' => [
        'aria_label' => 'Разделы настроек',
        'general' => 'Основные',
        'addresses' => 'Адреса',
        'delivery' => 'Доставка',
        'contacts' => 'Контакты',
        'menu' => 'Меню',
        'information' => 'Информация',
    ],

    'settings_information' => [
        'title' => 'Дополнительная информация',
        'description' => 'Текст показывается в карточке каждого товара на витрине',
        'fields' => [
            'description' => 'Текст',
        ],
        'placeholders' => [
            'description' => 'Введите текст, который увидят покупатели во всех карточках товаров...',
        ],
        'hints' => [
            'description' => 'Один текст на все карточки товаров. Показывается мелким шрифтом в блоке «Дополнительная информация».',
            'limit' => 'Использовано :current из :max символов',
        ],
        'actions' => [
            'save' => 'Сохранить',
            'saving' => 'Сохранение...',
        ],
        'messages' => [
            'updated' => 'Дополнительная информация обновлена',
        ],
        'errors' => [
            'too_long' => 'Текст слишком длинный (максимум :max символов)',
            'markup_too_long' => 'Слишком много форматирования — упростите оформление текста',
        ],
    ],

    'brand_type_request' => [
        'title' => 'Тип аккаунта',
        'change' => 'Изменить',
        'types' => [
            'master' => 'Мастер',
            'business' => 'Бизнес',
        ],
        'type_hints' => [
            'master' => 'Ремесленник или самозанятый. Покупатели связываются с вами напрямую.',
            'business' => 'ИП, ЧУП или ООО. Корзина, заказы и аналитика продаж.',
        ],
        'status' => [
            'pending' => 'Заявка на рассмотрении',
            'rejected' => 'Заявка отклонена',
        ],
        'pending_hint' => 'Запрошено: :type. Изменить тип можно после решения модератора.',
        'rejected_reason' => 'Причина',
        'dialog' => [
            'title' => 'Изменение типа аккаунта и реквизитов',
            'description' => 'Заявку рассмотрит модератор. До решения данные бренда не меняются.',
            'account_type' => 'Тип аккаунта',
            'employment_type' => 'Форма занятости',
            'employment_type_placeholder' => 'Выберите форму занятости',
            'submit' => 'Отправить',
            'submitting' => 'Отправка...',
            'cancel' => 'Отмена',
        ],
        'messages' => [
            'submitted' => 'Заявка на смену типа аккаунта отправлена',
        ],
        'errors' => [
            'not_found' => 'Бренд не найден',
            'already_requested' => 'Заявка на смену типа уже на рассмотрении',
            'nothing_changed' => 'Выбранные значения совпадают с текущими',
            'employment_type_mismatch' => 'Эта форма занятости не подходит для выбранного типа аккаунта',
        ],
    ],

    'settings_general' => [
        'title' => 'Основные настройки',
        'description' => 'Общая информация о вашем магазине',
        'fields' => [
            'about' => 'О бренде',
            'refund_policy' => 'Условия возврата',
            'image' => 'Логотип бренда',
            'employment_name' => 'Наименование бизнеса',
            'employment_number' => 'УНП',
        ],
        'placeholders' => [
            'about' => 'Расскажите о вашем бренде...',
            'refund_policy' => 'Опишите условия возврата товаров...',
            'employment_name' => 'Например, ООО «Ромашка»',
            'employment_number' => '123456789',
        ],
        'hints' => [
            'image' => 'Показывается покупателям рядом с названием бренда. Рекомендуем квадратную картинку.',
            'employment_number' => '9 цифр',
        ],
        'actions' => [
            'save' => 'Сохранить',
            'saving' => 'Сохранение...',
            'select_image' => 'Выбрать логотип',
            'change_image' => 'Заменить логотип',
            'remove_image' => 'Удалить',
        ],
        'messages' => [
            'updated' => 'Настройки обновлены',
        ],
        'errors' => [
            'image_not_owned' => 'Картинка не принадлежит вашему бренду',
            'employment_number_format' => 'УНП должен состоять из 9 цифр',
        ],
        'requisites' => [
            'title' => 'Реквизиты',
            'description' => 'Наименование и УНП вашего бизнеса',
            'employment_type' => 'Форма занятости',
        ],
    ],

    'settings_addresses' => [
        'title' => 'Адреса',
        'description' => 'Адреса вашего бизнеса',
        'new_address' => 'Новый адрес',
        'edit_address' => 'Редактирование адреса',
        'no_addresses' => 'Нет адресов',
        'loading' => 'Загрузка...',
        'fields' => [
            'name' => 'Название',
            'country' => 'Страна',
            'province' => 'Область / Регион',
            'city' => 'Город',
            'zip_code' => 'Индекс',
            'address1' => 'Адрес (строка 1)',
            'address2' => 'Адрес (строка 2)',
        ],
        'actions' => [
            'add' => 'Добавить адрес',
            'adding' => 'Добавление...',
            'edit' => 'Редактировать',
            'delete' => 'Удалить',
            'save' => 'Сохранить',
            'saving' => 'Сохранение...',
            'cancel' => 'Отмена',
        ],
        'messages' => [
            'added' => 'Адрес добавлен',
            'updated' => 'Адрес обновлён',
            'deleted' => 'Адрес удалён',
            'delete_error' => 'Не удалось удалить адрес',
        ],
        'confirm_delete' => 'Вы уверены, что хотите удалить этот адрес?',
    ],

    'settings_contacts' => [
        'title' => 'Контакты',
        'description' => 'Контактная информация вашего бизнеса',
        'fields' => [
            'email' => 'Email',
            'phone' => 'Телефон',
            'whatsapp' => 'WhatsApp',
            'telegram' => 'Telegram',
            'instagram' => 'Instagram',
            'facebook' => 'Facebook',
        ],
        'placeholders' => [
            'email' => 'info@example.com',
            'phone' => '+375 29 123-45-67',
            'whatsapp' => '+375291234567',
            'telegram' => '@username',
            'instagram' => '@username',
            'facebook' => 'https://facebook.com/page',
        ],
        'actions' => [
            'save' => 'Сохранить',
            'saving' => 'Сохранение...',
        ],
        'messages' => [
            'updated' => 'Контакты обновлены',
        ],
    ],

    'settings_delivery' => [
        'title' => 'Доставка',
        'description' => 'Выберите доступные способы доставки для вашего магазина',
        'no_methods' => 'Нет доступных способов доставки',
        'select_placeholder' => 'Введите значение и нажмите Enter',
        'actions' => [
            'save' => 'Сохранить',
            'saving' => 'Сохранение...',
        ],
        'messages' => [
            'updated' => 'Способы доставки обновлены',
        ],
        'errors' => [
            'too_long' => 'значение слишком длинное',
            'too_many_items' => 'слишком много значений в списке',
            'not_a_number' => 'значение должно быть числом',
        ],
    ],

    'shipping_constructors' => [
        'title' => 'Способы доставки',
        'description' => 'Создавайте универсальные шаблоны доставки и выбирайте поля для продавца и покупателя',
        'add' => 'Добавить способ',
        'no_constructors' => 'Пока нет ни одного способа доставки',
        'fields' => [
            'name' => 'Название способа доставки',
            'position' => 'Порядок',
            'description' => 'Описание',
            'active' => 'Активен',
        ],
        'merchant_fields' => [
            'title' => 'Поля продавца',
            'description' => 'Эти настройки заполняет магазин в своей вкладке доставки',
        ],
        'customer_fields' => [
            'title' => 'Поля покупателя',
            'description' => 'Эти поля будут нужны покупателю при оформлении заказа',
        ],
        'actions' => [
            'save' => 'Сохранить',
            'saving' => 'Сохранение...',
            'delete' => 'Удалить',
            'cancel' => 'Отмена',
        ],
        'delete_confirm' => [
            'title' => 'Удалить способ доставки?',
            'description' => 'Настройки этого способа у всех магазинов также будут удалены. Это действие необратимо.',
        ],
        'messages' => [
            'created' => 'Способ доставки создан',
            'updated' => 'Способ доставки обновлён',
            'deleted' => 'Способ доставки удалён',
        ],
    ],

    'impersonation' => [
        'impersonate' => 'Войти как',
        'stop' => 'Выйти из режима',
        'banner_text' => 'Вы вошли как',
    ],

    'category_picker' => [
        'label' => 'Категории',
        'hint' => 'Выберите категории',
        'search_placeholder' => 'Поиск категорий...',
    ],
    'option_picker' => [
        'label' => 'Опции',
        'hint' => 'Выберите опции, доступные для товаров в этой категории',
        'search_placeholder' => 'Поиск опций...',
        'loading' => 'Загрузка опций...',
        'empty' => 'Опции не найдены. Сначала создайте опции в разделе «Опции».',
    ],

    'settings_menu' => [
        'title' => 'Навигационное меню',
        'description' => 'Редактирование главного меню витрины. Перетаскивайте элементы, чтобы менять порядок и уровень вложенности.',
        'empty' => 'Меню пустое. Добавьте первый пункт.',
        'tab' => 'Меню',
        'fields' => [
            'title' => 'Заголовок',
            'link_type' => 'Тип ссылки',
            'url' => 'URL',
            'collection' => 'Коллекция',
            'category' => 'Категория',
        ],
        'link_types' => [
            'collection' => 'Коллекция',
            'category' => 'Категория',
            'custom' => 'Произвольная ссылка',
        ],
        'placeholders' => [
            'title' => 'Название пункта меню',
            'url' => 'https://… или /path',
            'pick_collection' => 'Выберите коллекцию',
            'pick_category' => 'Выберите категорию',
        ],
        'actions' => [
            'add_root' => 'Добавить пункт',
            'add_child' => 'Добавить подпункт',
            'delete' => 'Удалить',
            'save' => 'Сохранить меню',
            'saving' => 'Сохранение...',
            'expand' => 'Развернуть',
            'collapse' => 'Свернуть',
        ],
        'messages' => [
            'updated' => 'Меню обновлено',
            'max_depth' => 'Достигнута максимальная глубина.',
        ],
    ],

    'my_categories' => [
        'title' => 'Мои категории',
        'description' => 'Управление категориями товаров',
        'sections' => [
            'attached' => 'Ваши категории',
            'add' => 'Запросить категории',
            'history' => 'Мои заявки',
        ],
        'empty_attached' => 'К вашему бренду пока не привязано ни одной категории.',
        'empty_available' => 'Нет категорий, доступных для запроса.',
        'empty_history' => 'Заявок пока нет.',
        'form' => [
            'select_categories' => 'Выберите категории',
            'selected_categories' => 'Выбрано для заявки',
            'remove_category' => 'Убрать категорию из заявки',
            'submit' => 'Отправить заявку',
            'search_placeholder' => 'Поиск по названию...',
            'no_results' => 'Ничего не найдено',
        ],
    ],

    'category_requests' => [
        'title' => 'Запросы категорий',
        'description' => 'Заявки менеджеров на привязку категорий к их брендам.',
        'columns' => [
            'id' => 'ID',
            'brand' => 'Бренд',
            'manager' => 'Менеджер',
            'categories' => 'Категории',
            'status' => 'Статус',
            'created' => 'Создано',
        ],
        'statuses' => [
            'pending' => 'Ожидает',
            'approved' => 'Одобрено',
            'partial' => 'Частично',
            'rejected' => 'Отклонено',
        ],
        'actions' => [
            'view' => 'Открыть',
            'approve' => 'Принять',
            'reject' => 'Отклонить',
            'request' => 'Отправить заявку',
            'open_menu' => 'Открыть меню',
        ],
        'messages' => [
            'created' => 'Заявка отправлена.',
            'approved' => 'Позиции одобрены.',
            'rejected' => 'Позиции отклонены.',
        ],
        'errors' => [
            'no_brand' => 'У пользователя нет привязанного бренда.',
            'already_attached' => 'Некоторые категории уже привязаны к бренду.',
            'already_requested' => 'Некоторые категории уже находятся в активной заявке.',
        ],
        'reject_modal' => [
            'title' => 'Отклонить категории',
            'reason_label' => 'Причина (необязательно)',
            'reason_placeholder' => 'Почему отклоняем эти категории?',
            'submit' => 'Отклонить',
            'cancel' => 'Отмена',
        ],
        'show' => [
            'title' => 'Заявка на категории',
            'reviewed_at' => 'Обработано',
            'item_status' => 'Статус',
            'rejection_reason' => 'Причина отказа',
        ],
        'filters' => [
            'all_statuses' => 'Все статусы',
            'search_placeholder' => 'Поиск по бренду...',
            'columns' => 'Колонки',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'clear_selection' => 'Очистить выбор',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Заявок не найдено.',
    ],

    'moderation' => [
        'title' => 'Запросы брендов',
        'description' => 'Заявки от брендов на проверку: публикация страницы бренда и другие изменения.',
        'columns' => [
            'id' => 'ID',
            'source' => 'Бренд',
            'type' => 'Тип',
            'status' => 'Статус',
            'reviewer' => 'Проверил',
            'created' => 'Создано',
        ],
        'types' => [
            'brand_page' => 'Страница бренда',
            'brand_type' => 'Смена типа аккаунта',
        ],
        'statuses' => [
            'pending' => 'Ожидает',
            'approved' => 'Одобрено',
            'rejected' => 'Отклонено',
        ],
        'page_statuses' => [
            'none' => 'Не создана',
            'draft' => 'Не опубликована',
            'pending' => 'Заявка на рассмотрении',
            'published' => 'Опубликована',
        ],
        'actions' => [
            'view' => 'Открыть заявку',
            'open_source' => 'Открыть бренд',
            'open_menu' => 'Открыть меню',
        ],
        'source_deleted' => 'Бренд удалён',
        'show' => [
            'title' => 'Заявка',
            'request' => 'Заявка',
            'reviewed_at' => 'Рассмотрена',
            'reason' => 'Причина',
            'no_details' => 'Для этого типа заявки детали пока не настроены.',
            'empty_value' => 'Не заполнено',
            'actions' => [
                'title' => 'Решение по заявке',
                'description' => 'Одобрение опубликует страницу бренда, отказ вернёт её в черновик.',
                'reason' => 'Пояснение',
                'reason_placeholder' => 'Что нужно исправить бренду или почему заявка одобрена',
                'reason_hint' => 'При отказе пояснение обязательно — бренд увидит его в настройках.',
                'reason_required' => 'Укажите причину отказа.',
                'approve' => 'Одобрить',
                'reject' => 'Отклонить',
            ],
            'messages' => [
                'approved' => 'Заявка одобрена, страница бренда опубликована.',
                'rejected' => 'Заявка отклонена.',
            ],
            'errors' => [
                'already_processed' => 'Заявка уже обработана.',
                'source_missing' => 'Ресурс заявки удалён — решение принять нельзя.',
                'domain_required' => 'Сначала задайте домен бренда: без него адрес страницы не собрать.',
                'invalid_payload' => 'Данные заявки повреждены — решение принять нельзя.',
            ],
            'brand_type' => [
                'title' => 'Смена типа аккаунта',
                'previous' => 'Было',
                'requested' => 'Запрошено',
                'current' => 'Сейчас у бренда',
                'changed_since' => 'Бренд изменился после подачи заявки',
                'fields' => [
                    'type' => 'Тип аккаунта',
                    'employment_type' => 'Форма занятости',
                    'employment_name' => 'Наименование',
                    'employment_number' => 'УНП',
                ],
                'owner' => 'Владелец',
                'open_brand' => 'Открыть бренд',
            ],
            'brand' => [
                'title' => 'Бренд',
                'open_admin' => 'Открыть в админке',
                'open_page' => 'Открыть страницу',
                'preview_page' => 'Предпросмотр страницы',
                'domain_locked' => 'Домен уже назначен. Изменить его можно на странице бренда.',
                'page_status' => 'Статус страницы',
                'products' => 'Товаров',
                'categories' => 'Категорий',
                'about' => 'О бренде',
                'refund_policy' => 'Условия возврата',
                'owner' => 'Владелец и контакты',
                'owner_name' => 'Имя',
                'owner_email' => 'Email',
                'owner_phone' => 'Телефон',
                'no_owner' => 'К бренду не привязан пользователь.',
            ],
        ],
        'filters' => [
            'all_statuses' => 'Все статусы',
            'all_types' => 'Все типы',
            'search_placeholder' => 'Поиск по бренду...',
            'columns' => 'Колонки',
        ],
        'bulk' => [
            'selected' => 'выбрано',
            'clear_selection' => 'Очистить выбор',
        ],
        'pagination' => [
            'rows_selected' => 'из',
            'row' => 'строк',
            'rows_per_page' => 'Строк на странице',
            'page' => 'Страница',
            'of' => 'из',
            'go_first' => 'На первую страницу',
            'go_previous' => 'На предыдущую страницу',
            'go_next' => 'На следующую страницу',
            'go_last' => 'На последнюю страницу',
        ],
        'empty' => 'Запросов от брендов не найдено.',
    ],

    'errors' => [
        'page_expired' => 'Страница устарела: сессия истекла. Войдите ещё раз и повторите действие.',
    ],

    'my_page' => [
        'title' => 'Моя страница',
        'description' => 'Персональная страница вашего бренда',
        'statuses' => [
            'draft' => [
                'title' => 'Не опубликована',
                'description' => 'Отправьте заявку на публикацию. После одобрения вы сможете открыть страницу и поделиться ссылкой.',
            ],
            'pending' => [
                'title' => 'Заявка на рассмотрении',
                'description' => 'Мы сообщим, как только модератор рассмотрит заявку.',
            ],
            'published' => [
                'title' => 'Опубликована',
                'description' => 'Страница доступна. Делитесь ссылкой со своей аудиторией.',
            ],
            'rejected' => [
                'title' => 'Заявка отклонена',
                'description' => 'Исправьте замечания и отправьте заявку повторно.',
            ],
        ],
        'url' => [
            'published' => 'Ссылка на вашу страницу',
            'preview' => 'Пример адреса вашей страницы',
            'empty' => 'Адрес страницы ещё не назначен. Обратитесь в поддержку.',
        ],
        'actions' => [
            'open_page' => 'Открыть страницу',
            'copy_link' => 'Скопировать ссылку',
            'brand_settings' => 'Настройки бренда',
            'submit' => 'Отправить заявку',
            'resubmit' => 'Отправить заявку повторно',
            'cancel' => 'Отмена',
        ],
        'dialog' => [
            'title' => 'Отправить заявку на публикацию?',
            'description' => 'После одобрения страница бренда, товары и указанные вами контакты станут доступны всем посетителям SPRA.BY. До этого страница останется закрытой.',
            'terms_link' => 'Условия публикации и ответственность продавца',
            'consent' => 'Подтверждаю, что уполномочен публиковать страницу бренда, принимаю Условия публикации и отвечаю за достоверность сведений, законность материалов и товаров, а также исполнение обязательств перед покупателями. Прошу SPRA.BY разместить выбранные материалы и контактные данные в открытом доступе.',
        ],
        'published_at' => 'Опубликована',
        'sent_at' => 'Отправлена',
        'reviewed_at' => 'Рассмотрена',
        'reason' => 'Причина',
        'copied' => 'Ссылка скопирована',
        'empty' => 'Бренд не найден. Обратитесь в поддержку.',
        'messages' => [
            'submitted' => 'Заявка отправлена на рассмотрение.',
        ],
        'errors' => [
            'not_found' => 'Бренд не найден.',
            'already_published' => 'Страница бренда уже опубликована.',
            'already_requested' => 'Заявка уже отправлена и ожидает рассмотрения.',
        ],
    ],
];
